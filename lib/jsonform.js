...
(function ($) {
  ...
  $.fn.extend({
    ...
    jsonform: function (json, callback) {
    	...
    	var _json = {}, 
    	    _form = this, 
    	    _callback = function(json) { _json = json; };
    	    
    	var _populate = function(json) {
  ...
  /*
      ...
      var _getJSON = function() {
        var json = {};
        _form.find("input,textarea,select").each(function(index, item){
          _form.merge(json, buildJson(item.id, item.value));
        });
        return json;
      };
    	
      if(typeof json === "function") {
      	_callback = json; json = null;
    	} else { 
    	  _json = json;
    	}
